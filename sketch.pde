...
**/
var scene = 0;
var bgColors = [100, 100, 100];
var mode = 1;
var winners = [["Casual Clicker", 10],["SirArthur09", 100], ["Gregory Smith", 777], ["Walt_Narnia", 1015], ["rgpeep", 3034]];
var messages = [
...
var tri = function(x, y, size, color){
    this.x = x;
    this.y = y;
    this.color = color;
    this.alpha = 255;
    this.size = size;
    this.rotationEffect = random(0, 4);
    this.rotate = 0;
    this.acceleration = [random(-2, 2), random(-3, 1)];
};
tri.prototype.display = function(){
    if(this.x > 0 && this.x < 400 && this.y > 0 && this.y < 400){
        this.rotate+=this.rotationEffect;
        this.x+=this.acceleration[0];
        this.y+= this.acceleration[1];
        if (this.alpha >1){
            this.alpha -= 2;
        }
        this.acceleration[1] += 0.03;
        if(this.acceleration[1] < 0){
            this.acceleration[1] += 0.02;
        }
...
var tris = [new tri(405, 200, 0, [0, 0, 0])];
...
var winAnimation = function(x, y){
    if(tris.length === 0){
        for(var i = 0; i<50; i++){
        var myTri = new tri(x, y, random(5, 10), [random(100, 255), random(100, 255), random(100, 255)]);
        tris.push(myTri);
    }
    }
};
...
var game = function(){
    ...
    for(var i = 0; i<winners.length; i++){
        if (winners[i][1] === clicks){
            theWinner = winners[i][0];
        }
    }
    ...
    if (theWinner !== ""){
        if(won !== true){
            winAnimation(200, 130);
            won = true;
        }
        text("Congragulations!\nYou've beat "+theWinner+"!", 200, 130);
    }else{
        won = false;
    }//Stop showing a Winner
    if(tris[0].alpha !== 1){
        for(var i = 0; i<tris.length; i++){
            tris[i].display();
        }
    } //Still show animation if not complete
    }//Beaten alert+Win animation
};
var extra = function(){
    draw_bg();
    fill(0);
...
var note = function(){
    ...
    fill(0);
    textSize(20);
    text("If you want to adjust \nhow soon the upcoming message alert \nshows you can do so \nin the Extra tab in the menu. \nYou can also turn it off by setting it to zero.", 200, 200);
};
...
var draw = function() {
    switch(scene){
        case 0:
            //background(255, 0, 0); winAnimation(200, 200);
            main();
            break;
        case 1:
            game();
